...
BEGIN
	...
	METRICS_CTE AS (
		SELECT		WellID, 
					...
					SUM(CASE WHEN MonthRankDesc_GW <= 12 AND DateRankAsc > 3 THEN COALESCE(WATER, 0.0) END) / NULLIF(SUM(CASE WHEN MonthRankDesc_GW <= 12 AND DateRankAsc > 3 THEN COALESCE(GAS, 0.0) / 1000 END), 0.0) AS WTR_YIELD
		FROM		PROD_PIVOT_CTE
		GROUP BY	WellID
	)
